...
class pose_deviation_plugin : public plugin {
public:
    pose_deviation_plugin(const std::string& name, phonebook* pb)
    	: plugin{name, pb}
	{
		pb->register_impl<pose_prediction>(
			std::static_pointer_cast<pose_prediction>(
				std::make_shared<pose_deviation_impl>(pb)
			)
		);
	}
};

PLUGIN_MAIN(pose_deviation_plugin);
