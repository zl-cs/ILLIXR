...
def threading_imap_unordered(
    ...
    """
    results: queue.Queue[Tuple[str, Union[BaseException, V]]] = queue.Queue(
        maxsize=operator.length_hint(iterable)
    )

    def worker(
        chunk: List[T], results: queue.Queue[Tuple[str, Union[BaseException, V]]]
    ) -> None:
        try:
            for elem in chunk:
    ...
    def get_results(
        results: queue.Queue[Tuple[str, Union[BaseException, V]]]
    ) -> Iterable[V]:
        while any(thread.is_alive() for thread in threads):
            try:
                ...
                if result[0] == "elem" and not isinstance(result[1], BaseException):
                    ...
                    yield result[1]
                elif result[0] == "exception" and isinstance(result[1], BaseException):
                    raise result[1]
                else:
    ...
    results_list = list(
        tqdm(
            get_results(results), total=my_length_hint(iterable, length_hint), desc=desc,
        )
    )
