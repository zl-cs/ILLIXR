...
public:
     ...
     */
    auto operator==(const record_header& other) const -> bool
    {
        // Check pointer first
        if (this == &other) {
            return true;
    ...
    auto               operator!=(const record_header& other) const -> bool { return !(*this == other); }
    [[nodiscard]] auto get_id() const -> std::size_t { return id; }
    [[nodiscard]] auto get_name() const -> const std::string& { return name; }
    [[nodiscard]] auto get_column_name(unsigned column) const -> const std::string& { return columns[column].first; }
    [[nodiscard]] auto get_column_type(unsigned column) const -> const std::type_info& { return columns[column].second; }
    [[nodiscard]] auto get_columns() const -> unsigned { return columns.size(); }
...
{
public:
    data_use_indicator() = default;
    data_use_indicator(const data_use_indicator& other) { other.used = true; }
    auto operator=(const data_use_indicator& other) -> data_use_indicator&
    {
        if (&other != this) {
            other.used = true;
            used       = false;
...
public:
    ...
    template <typename T> auto get_value(unsigned column) const -> T
    {
#ifndef NDEBUG
        assert(rh);
    ...
    auto get_record_header() const -> const record_header&
    {
        assert(rh);
        return rh->get();
    }
...
public:
     ...
     */
    auto get(std::size_t namespace_ = 0, std::size_t subnamespace = 0, std::size_t subsubnamespace = 0) -> std::size_t
    {
...
static constexpr std::chrono::milliseconds LOG_BUFFER_DELAY { 1000 };
