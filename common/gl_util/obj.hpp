        ...
        if (!success) {
...
#ifndef NDEBUG
                std::cout << "[OBJ INFO] Loading material named: " << material.name << std::endl;
                std::cout << "[OBJ INFO] Material texture name: " << material.diffuse_texname << std::endl;
#endif
            ...
            for (auto& shape : shapes) {
#ifndef NDEBUG
                std::cout << "[OBJ INFO] Num verts in shape: " << shape.mesh.indices.size() << std::endl;
                std::cout << "[OBJ INFO] Num tris in shape: " << shape.mesh.indices.size() / 3 << std::endl;
#endif
                // Unified buffer for pos + uv. Interleaving vertex data (good practice!)
                std::vector<vertex_t> buffer;
                // Iterate over triangles
                for (size_t tri_idx = 0; tri_idx < shape.mesh.indices.size() / 3; tri_idx++) {
                    tinyobj::index_t idx0 = shape.mesh.indices[3 * tri_idx + 0];
                    tinyobj::index_t idx1 = shape.mesh.indices[3 * tri_idx + 1];
                    tinyobj::index_t idx2 = shape.mesh.indices[3 * tri_idx + 2];
                ...
                if (shape.mesh.material_ids.size() >= 0) {
                    std::string texname = materials[shape.mesh.material_ids[0]].diffuse_texname;
