 ...
 */
template <typename event> class reader_latest
{
public:
    /**
     * @brief Gets a "read-only" copy of the latest value.
     */
    virtual auto get_latest_ro() const -> const event* = 0;

    /**
     * @brief Gets a mutable copy of the latest value.
     */
    virtual auto get_latest() const -> event* = 0;

    virtual ~reader_latest() = default;
    ;
};
 ...
 */
template <typename event> class writer
{
public:
    /**
...
private:
    virtual auto _p_publish(const std::string& topic_name, std::size_t ty) -> std::unique_ptr<writer<void>> = 0;

    virtual auto _p_subscribe_latest(const std::string& topic_name, std::size_t ty) -> std::unique_ptr<reader_latest<void>> = 0;

    virtual void _p_schedule(std::size_t component_id, const std::string& topic_name, std::function<void(const void*)> fn,
                             std::size_t ty)
        = 0;
...
public:
    ...
    {
        _p_schedule(
            component_id, topic_name, [=](const void* ptr) { fn(reinterpret_cast<const event*>(ptr)); },
     ...
     * @throws If topic already exists, and its type does not match the @p event.
     */
    template <typename event> auto publish(const std::string& topic_name) -> std::unique_ptr<writer<event>>
    {
     ...
     * @throws If topic already exists, and its type does not match the @p event.
     */
    template <typename event> auto subscribe_latest(const std::string& topic_name) -> std::unique_ptr<reader_latest<event>>
    {
