 ...
 *
 */
static inline auto cpp_clock_gettime(clockid_t clock_id) -> std::chrono::nanoseconds
{
 ...
 */
static inline auto thread_cpu_time() -> std::chrono::nanoseconds { return cpp_clock_gettime(CLOCK_THREAD_CPUTIME_ID); }
...
auto count_duration(Duration t) -> typename std::enable_if<std::is_integral<Out>::value, Out>::type
{
    return std::chrono::duration_cast<std::chrono::nanoseconds, typename Duration::rep, typename Duration::period>(t).count();
}

template <typename Duration>
auto count_duration(Duration t) -> typename std::enable_if<std::is_integral<Duration>::value, Duration>::type
{
    return t;
}
...
private:
    ...
    public:
        ...
        ~print_in_destructor()
        {
            ...
            constexpr int MAGIC_NUMBER = 100;
            if (rand() % MAGIC_NUMBER == 0) {
#ifndef NDEBUG
...
static auto gen_serial_no() -> std::size_t
{
...
public:
    should_profile_class()
    {
        ...
        actually_should_profile           = (ILLIXR_STDOUT_METRICS != nullptr) && (strcmp(ILLIXR_STDOUT_METRICS, "y") == 0);
    }
    auto operator()() const -> bool { return actually_should_profile; }
...
auto timed_thread(const std::string& account_name, Function&& f, Args&&... args) -> std::thread
{
