...
load_data(const std::string& data_path) {
	...
	if (!imu0_file.good()) {
		std::cerr << "'" << data_path << imu0_subpath << "' is not a good path"
		          << std::endl;
		abort();
	}
	...
	if (!cam0_file.good()) {
		std::cerr << "'" << data_path << cam0_subpath << "' is not a good path"
		          << std::endl;
		abort();
	}
	...
	if (!cam1_file.good()) {
		std::cerr << "'" << data_path << cam1_subpath << "' is not a good path"
		          << std::endl;
		abort();
	}
