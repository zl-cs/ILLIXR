...
load_data() {
	...
	if (!imu0_file.good()) {
		std::cerr << "'" << illixr_data << imu0_subpath << "' is not a good path"
		          << std::endl;
		abort();
	}
	...
	if (!cam0_file.good()) {
		std::cerr << "'" << illixr_data << cam0_subpath << "' is not a good path"
		          << std::endl;
		abort();
	}
	...
	if (!cam1_file.good()) {
		std::cerr << "'" << illixr_data << cam1_subpath << "' is not a good path"
		          << std::endl;
		abort();
	}
