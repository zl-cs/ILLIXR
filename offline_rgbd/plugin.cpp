...
class offline_rgbd : public threadloop {
public:
    offline_rgbd(std::string name_, phonebook* pb_)
        : threadloop{name_, pb_}
        , sb{pb->lookup_impl<switchboard>()}
        , _m_rgbd_publisher{sb->get_writer<rgb_depth_type>("rgb_depth")}
    ...
    virtual void _p_one_iteration() override {
        ...
        if (last_ts != nearest_row->first) {
            ...
            auto rgb_im = nearest_row->second.rgb.load();
            auto depth_im = nearest_row->second.depth.load_depth();

            time_point expected_real_time_given_dataset_time(
                std::chrono::duration<long, std::nano>{nearest_row->first - dataset_first_time});
                
            _m_rgbd_publisher.put(_m_rgbd_publisher.allocate(expected_real_time_given_dataset_time, rgb_im, depth_im));

            // _m_rgbd_publisher.put(_m_rgbd_publisher.allocate<rgb_depth_type>(rgb_depth_type{
...
private:
    ...
    switchboard::writer<rgb_depth_type>            _m_rgbd_publisher;
